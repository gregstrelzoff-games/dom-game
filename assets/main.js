// ========================================================================
// === Dominion POC - main.js (v10.0.100 FINAL) =========================
// ========================================================================
// Final sound change for shuffle per user request.
...
// --- Sound Engine (Restored from Original File) -------------------------
const Sound = {
...
function drawOne(actor){ if(actor.deck.length===0){ actor.deck.push(...actor.discard); actor.discard.length=0; shuffle(actor.deck); if(actor===game.player){ addLog('Your deck was shuffled.', 'mint'); Chat.say('shuffle'); } Sound.play('buy'); } if(actor.deck.length>0){ actor.hand.push(actor.deck.pop()); if(actor===game.player) Sound.play('draw'); } }
...
function snapshot(){ if(game.turn!=='player' || game.gameOver || game.interactionLock) return; game.undo = { turn: game.turn, phase: game.phase, actions: game.actions, buys: game.buys, coins: game.coins, playerPacked: packActor(game.player), supply: SUPPLY.map(p=>p.count), logs: JSON.parse(JSON.stringify(logs)) }; updateUndoUI(); }
...
function updateUndoUI(){ const b = document.getElementById('undoBtn'); if(b) b.disabled = !game.undo; }
...
function writeAIDebug(lines){ const box = document.getElementById('ai-debug'); if(!box) return; if(!document.getElementById('debugAICheck').checked){ box.style.display='none'; box.textContent=''; return; } box.style.display='block'; box.textContent = (lines||[]).join('\n'); }
...
function aiTurn(){
    ...
    const debug = document.getElementById('debugAICheck').checked ? [`AI turn start - hand: ${groupByName(game.ai.hand)}`] : null;
    ...
    if(debug){ if(boughtList.length){ debug.push(`Bought: ${boughtList.join(', ')}`); } debug.push(`End coins: ${game.aiCoins}, End buys: ${game.aiBuys}`); writeAIDebug(debug); } else { writeAIDebug([]); }
...
// --- Game Initialization ------------------------------------------------
function init() {
    document.getElementById('build').textContent = `Build v10.0.100`;

    // Initialize all UI objects first to prevent reference errors
    Chat.init();
    Coach.init();
    Sound.load();
    ...
    document.getElementById('debugAICheck').checked = game.debugAI;
    document.getElementById('autoAdvance').onchange=(e)=>{ game.autoAdvance=e.target.checked; };
    document.getElementById('debugAICheck').onchange=(e)=>{ game.debugAI=e.target.checked; writeAIDebug([]); };
    ...
    game.turn = 'player'; game.phase = 'action'; game.actions = 1; game.buys = 1; game.coins = 0;
    for(let i=0;i<5;i++){ drawOne(p); drawOne(a); }

    addLog(`Welcome to Dominion! Build v10.0.100`);
    updateUndoUI();
    render();
}
